          ...
          <div>
            ...
            <div className="mt-6 grid auto-rows-fr gap-3 text-xs text-slate-300 sm:grid-cols-3">
              {highlights.map((item) => (
                <div
                  key={item.label}
                  className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/5 bg-gradient-to-b from-slate-900/70 to-black/80 p-3 shadow-[0_10px_25px_rgba(0,0,0,0.45)]"
                >
              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-2 text-[11px] sm:grid-cols-2">
                  {loopFlow.map((step) => (
                    <div
                      key={step.text}
                      className="flex items-center justify-between rounded-xl border border-white/5 bg-black/40 px-3 py-2 text-slate-300"
                    >
                      <span>{step.text}</span>
                      <span
